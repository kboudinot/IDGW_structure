# Population structure and genetic diversity of gray wolves in Idaho 25 years after reintroduction  
## Repository for code associated with analysis of population structure and genetic diversity for gray wolves in Idaho using single nucleotide polymorphism (SNP) and microhaplotype data generated by Genotyping-in-Thousands by sequencing (GT-seq).
Authors: Koryna Boudinot, Elise M. Stacy, Ariana L. Cerreta, Joel M. Tebbenkamp, and David E. Ausband 
### Primary contact: Koryna Boudinot, kboudinot@uidaho.edu

## Genotyping and bioinformatics
Cerreta et al. in review pipeline: https://github.com/arianacerreta/IDGW-bioinformatics-pipeline

Delomas et al. 2023 pipeline: https://github.com/arianacerreta/IDGW-bioinformatics-pipeline

## R scripts
### filtering_microhaps.R

Data inputs: 
1. microhaplotype_genotypes_raw.csv
2. neutral_diagnostic_loci_new(in).csv
3. sample_metadata_DAU.csv
4. relatedness_trioml.csv

Details: Script used for filtering data based on missingess per locus and genotype, diagnostic loci, replicates, and relatedness.

Output: structure_data.txt

## References 
Cerreta, A. L., J. R. Adams, K. Petersen, N. R. Campbell, and D. E. Ausband. In review. More than 	one way to SNP a scat? The implementation of a microhaplotype SNP panel for non-	invasive fecal samples. G3: Genes | Genomes | Genetics. 

Delomas, T. A., J. Struthers, T. Hebdon, and M. R. Campbell. 2023. Development of a microhaplotype panel to inform management of gray wolves. Conservation Genetics Resources 15:49–57.

